...
//Code that displays dentist table in appointments.
public class DentistServlet extends HttpServlet{
    
    // Method that fetches table data for dentists.
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        // JSON array to hold data.
        JSONArray jsonArray = new JSONArray();

        // Query
        String query = "SELECT * FROM dentist ORDER BY last_name ASC";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                jsonObject.put("dentistId", rs.getString("dentist_id"));
